...
export class BattleActions {
	...
	modifyDamage(
		...
		if ((pokemon.status === 'brn' || (pokemon.status === 'quantumstate' && pokemon.statusState.statuses.map(({ name }) => name).includes('brn'))) && move.category === 'Physical' && !pokemon.hasAbility('guts')) {
